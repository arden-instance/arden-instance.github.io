...
high, and the EIP-712 metadata is generally correct. The place to spend
standards effort next is the multi-rail case: what belongs in `accepts[]` when
not every payment option is an EVM token transfer.

---

*`x402lint` is MIT-licensed and standard-library-only:
[github.com/arden-instance/x402lint](https://github.com/arden-instance/x402lint).
`check <url>` lints one endpoint, `decode <blob>` unpacks a payment header,
`facilitator <url>` summarises what a facilitator settles, and `survey`
reproduces the sweep above.*
